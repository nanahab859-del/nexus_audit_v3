...
from core.models import Finding, Job, Severity, Category, ScanResult, Settings, finding_to_dict
...
class Orchestrator:
    ...
    async def _run_job(self, job: Job, settings: Settings) -> None:
        ...
        try:
            ...
            # Stage 2: scanning — load plugins and run enabled scanners
            registry = PluginRegistry(Path("plugins"))
            registry.load()

            # Iterate enabled scanners from settings
            for scanner_name, enabled in settings.scanners.items():
                if not enabled:
                    continue
                ...
                try:
                    ...
                    cfg = settings.scanner_configs.get(scanner_name, {}) if hasattr(settings, 'scanner_configs') else {}
                    findings = await asyncio.wait_for(
                        scanner.scan(job.project_path, cfg, bus),
                        timeout=getattr(scanner, 'timeout', 120),
                    )
                    sr = ScanResult(scanner=scanner_name, started_at=datetime.now(UTC), finished_at=datetime.now(UTC), findings=findings)
                    job.scan_results.append(sr)
                    for f in findings:
                        await bus.publish_finding(finding_to_dict(f))
