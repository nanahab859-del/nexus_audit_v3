...
export function setDataFromJob(jobData) {
  ...
  // Parse findings
  findings.forEach(f => {
    if (!scannerSummary[f.scanner]) {
      scannerSummary[f.scanner] = { count: 0, duration: null };
    }
    scannerSummary[f.scanner].count++;
  });
  
  // Extract health scores from apps
  const healthScores = {};
  const appsData = jobData.apps || {};
  for (const [app, appData] of Object.entries(appsData)) {
    if (appData && typeof appData === 'object') {
      healthScores[app] = appData.score !== undefined ? Math.round(appData.score) : 0;
    }
  }
  
  // Extract ghost files
  const ghostFiles = (jobData.ghost_files || []).map(f => f.file || f.path || f);
  
  _update({
    job: jobData,
    findings: findings,
    scannerSummary: scannerSummary,
    architecture: jobData.dna || null,
...
export function appendFinding(f) {
  const list = [..._state.findings, f].slice(-1000);
  _update({ findings: list });
}
