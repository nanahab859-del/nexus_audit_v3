...
async def test_mcp_integration_cycle(tmp_path, monkeypatch):
    ...
    res = await mcp.call_tool("run_project_audit", {"project_id": "test-proj", "fast_mode": True})
    # Since there's no project registered in this bare test, it will probably return an error string
    assert "Error" in str(res)
    
    # 3. List
    res_list = await mcp.call_tool("list_findings", {"project_id": "test-proj", "job_id": "job-123"})
    assert getattr(res_list, "content", []) == [] or res_list == []
    
    # 4. Detail
    res_detail = await mcp.call_tool("get_finding_detail", {"project_id": "test-proj", "job_id": "job-123", "fingerprint": "fp1"})
