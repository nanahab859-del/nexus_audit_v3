...
from core.infra.audit_index import open_index, upsert_run, rebuild_index, get_trend, get_fix_queue, diff_runs
...
async def test_upsert_run_inserts_row(project_id, monkeypatch, tmp_path):
    ...
    assert row["run_id"] == "job1"
    assert row["score_overall"] == 95.5
...
async def test_rebuild_index_produces_rows(project_id, monkeypatch, tmp_path):
    ...
    for i in range(3):
        ...
        summary = {
            "job_id": f"job{i}",
            "timestamp": f"2023-10-01T12:0{i}:00Z",
            "fleet_average": 100.0,
            "findings_count": 0
        }
