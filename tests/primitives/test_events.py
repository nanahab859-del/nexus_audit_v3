...
async def test_unsubscribe_then_resubscribe():
    ...
    token1 = bus.subscribe(EventType.LOG, cb1)
    bus.unsubscribe(token1)
    bus.subscribe(EventType.LOG, cb2)
...
async def test_safe_notify_error_handling(caplog):
    ...
    bus.subscribe(EventType.LOG, bad_sub)
    with caplog.at_level(logging.WARNING):
...
async def test_convenience_methods():
    ...
    # Mock to check calls
    events = []
    bus.subscribe_all(lambda id, evt: events.append(evt))
...
async def test_publish_status_convenience_method():
    bus = EventBus()
    received = []
    bus.subscribe(EventType.STATUS, lambda evt: received.append(evt))
...
async def test_publish_finding_convenience_method():
    bus = EventBus()
    received = []
    bus.subscribe(EventType.FINDING, lambda evt: received.append(evt))
...
async def test_unsubscribe_all_tokens():
    ...
    t1 = bus.subscribe(EventType.LOG, lambda evt: calls1.append(evt))
    t2 = bus.subscribe(EventType.STATUS, lambda evt: calls2.append(evt))
    t3 = bus.subscribe_all(lambda id, evt: calls3.append(evt))
...
async def test_unsubscribe_invalid_token():
    bus = EventBus()
    bus.unsubscribe("invalid-token") # Should not raise
...
async def test_unsubscribe_remove_failed():
    ...
    token = bus.subscribe(EventType.LOG, cb)
    bus._subscribers[EventType.LOG].remove(cb) 
    bus.unsubscribe(token) # Should not raise
